...
| Path | Role |
|------|------|
| `eval/README.md` | Index of the three eval suites and `guardrail_smoke.py`. |
| `eval/guardrail_smoke.py` | Fast guardrail routing checks (no BigQuery). |
| `eval/GoldenDatasetEval/golden_dataset.json` | Golden Q&A pairs (`id`, `question`, `expected_sql_pattern`, `validation`; optional `source`). |
| `eval/GoldenDatasetEval/run_eval.py` | Runs each golden item through `ask()`, pattern-checks SQL, scores with `judge.py`, writes `eval/GoldenDatasetEval/results.json`. |
| `eval/GoldenDatasetEval/judge.py` | LLM-as-judge for single-turn golden eval (Gemini). |
| `eval/LangsmithEval/langsmith_account_check.py` | Prints workspace / project URL for the API key in `.env`. |
| `eval/LangsmithEval/verify_langsmith.py` | Lists recent LangSmith runs; optional `--smoke`. |
| `eval/LangsmithEval/run_langsmith_checks.py` | Writes `eval/LangsmithEval/langsmith_report.json` (workspace + recent runs). |
| `eval/ConversationEval/conversation_stress_dataset.json` | Multi-turn stress scenarios. |
| `eval/ConversationEval/run_conversation_eval.py` | Full conversation suite → `conversation_results.json`. |
| `eval/ConversationEval/run_education_conversation_eval.py` | Single scenario by `--id` → `education_conversation_results.json`. |
| `eval/ConversationEval/conversation_judge.py` | LLM judge for whole transcripts (1-5 + reasoning). |
| `eval/GoldenDatasetEval/results.json` | Generated by golden eval. |
| `eval/ConversationEval/conversation_results.json` | Generated by conversation stress eval. |
| `eval/LangsmithEval/langsmith_report.json` | Generated by `run_langsmith_checks.py`. |
